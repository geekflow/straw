...
type Client interface {
	Write(context.Context, []internal.Metric) error
...
func (i *InfluxDB) Write(metrics []internal.Metric) error {
	ctx := context.Background()
	...
	for _, n := range p {
		...
		case *DatabaseNotFoundError:
			if !i.SkipDatabaseCreation {
				...
				if err != nil {
					...
					log.Errorf("When writing to [%s]: database %q not found and failed to recreate",
						client.URL(), apiError.Database)
				}
			}
		}
		...
		log.Errorf("When writing to [%s]: %v", client.URL(), err)
	}
