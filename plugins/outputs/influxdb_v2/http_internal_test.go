...
func TestMakeWriteURL(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			act: "http://localhost:9999/api/v2/write?bucket=straw&org=influx",
		},
		{
			url: genURL("unix://var/run/influxd.sock"),
			act: "http://127.0.0.1/api/v2/write?bucket=straw&org=influx",
		},
		{
			err: true,
	...
	for i := range tests {
		rURL, err := makeWriteURL(*tests[i].url, "influx", "straw")
		if !tests[i].err {
